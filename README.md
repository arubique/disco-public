...
The sections below include: [**Installation**](#installation) for environment setup; [**Reproducing results from the paper**](#reproduce-results-from-the-paper) to generate Table 1 from the paper; [**Download model outputs**](#download-model-outputs) to fetch the required data; [**Train DISCO**](#train-disco) for training our method and the baselines; [**Notes on `stnd.run_from_csv.py` and `.csv` files**](#note-about-stndrun_from_csvpy-and-csv-files) for details on the experiment runner used to organize experiments; and [**Citation**](#citation).
